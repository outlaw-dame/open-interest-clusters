...
export class RemoteDatasetFetchError extends Error { constructor(message: string) { super(message); this.name = "RemoteDatasetFetchError"; } }
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      ...
      if (response.status === 304) {
        if (!options.cache?.dataset) throw new RemoteDatasetFetchError("Received 304 Not Modified without a cached dataset.");
        return withOptionalEtag(options.cache.dataset, options.cache.etag, true);
      }
      if (!response.ok) throw new RemoteDatasetFetchError(`Remote dataset fetch failed with status ${response.status}.`);
      ...
      lastError = error;
      if (options.signal?.aborted === true || attempt == attempts) break;
