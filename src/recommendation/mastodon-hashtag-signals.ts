...
function authorize(value: unknown, subjectId: string, authenticated: boolean): void {
  ...
  if (authenticated && value.accessBasis !== "oauth_scope" && value.accessBasis !== "authenticated_api") throw new TypeError("Mastodon hashtag source requires authenticated authorization evidence.");
  if (!authenticated && value.accessBasis === "unknown") throw new TypeError("Mastodon hashtag source requires public-read authorization evidence.");
}

function history(value: unknown): { uses: number; accounts: number } {
  if (value === undefined) return { uses: 0, accounts: 0 };
  if (!Array.isArray(value) || value.length > 14) throw new TypeError("Invalid Mastodon hashtag history.");
  let uses = 0;
  let accounts = 0;
...
function parseItems(input: { body: unknown; kind: RecommendationMastodonHashtagSignalKind; observedAt: string; baseUrl: URL; accountId?: string }): readonly RecommendationMastodonHashtagEvidence[] {
  if (!Array.isArray(input.body) || input.body.length > MAX_TAGS) throw new TypeError("Invalid Mastodon hashtag response.");
  return Object.freeze(input.body.map((raw): RecommendationMastodonHashtagEvidence => {
    ...
    const aggregate = history(raw.history);
...
function client(input: { baseUrl: string; path: string; authenticated: boolean; kind: RecommendationMastodonHashtagSignalKind; transport: RecommendationMastodonHashtagTransport; accountId?: string }) {
  ...
  return Object.freeze({
    async read(readInput: { subjectId: string; authorization: RecommendationProtocolSourceReadAuthorization; signal?: AbortSignal }) {
      ...
      authorize(readInput.authorization, subjectId, input.authenticated);
      const response = await input.transport.get({ url: url.toString(), requiresAuthentication: input.authenticated, ...(readInput.signal === undefined ? {} : { signal: readInput.signal }) });
      if (!record(response)) throw new TypeError("Invalid Mastodon hashtag transport response.");
      return parseItems({ body: response.body, kind: input.kind, observedAt: instant(response.observedAt, "observation timestamp"), baseUrl, ...(input.accountId === undefined ? {} : { accountId: input.accountId }) });
    }
  });
}
...
export function createRecommendationMastodonAccountFeaturedTagsClient(input: { baseUrl: string; accountId: string; transport: RecommendationMastodonHashtagTransport }) {
  ...
  return client({ ...input, accountId, path: `/api/v1/accounts/${encodeURIComponent(accountId)}/featured_tags`, authenticated: false, kind: "account_featured" });
}
...
export function createRecommendationMastodonViewerFeaturedTagsClient(input: { baseUrl: string; transport: RecommendationMastodonHashtagTransport }) {
  return client({ ...input, path: "/api/v1/featured_tags", authenticated: true, kind: "viewer_featured" });
}
...
export function createRecommendationMastodonFollowedTagsClient(input: { baseUrl: string; transport: RecommendationMastodonHashtagTransport; limit?: number }) {
  ...
  return client({ ...input, path: `/api/v1/followed_tags?limit=${limit}`, authenticated: true, kind: "viewer_followed" });
}
...
export function createRecommendationMastodonTrendingTagsClient(input: { baseUrl: string; transport: RecommendationMastodonHashtagTransport; limit?: number }) {
  ...
  return client({ ...input, path: `/api/v1/trends/tags?limit=${limit}`, authenticated: false, kind: "instance_trending" });
}
