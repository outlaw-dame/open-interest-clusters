...
export interface RecommendationCandidateEligibilityInput {
  ...
  providerPolicy: RecommendationCandidatePolicyGate;
  viewerSafety: RecommendationCandidateViewerSafetyGate;
  evaluatedAt?: string;
  signal?: AbortSignal;
}
...
const INPUT_KEYS = new Set(["candidate", "evidence", "providerPolicy", "viewerSafety", "evaluatedAt", "signal"]);
...
const EVIDENCE_KEYS: Readonly<Record<RecommendationCandidateKind, ReadonlySet<string>>> = {
  account: new Set(["kind", "resolver", "identityBindingVerified", "discoverable", "noindex", "optedOut", "inactivityDays"]),
...
function normalizeEvidence(
  ...
  switch (expectedKind) {
    case "account": {
      if (!isRecord(value.resolver) || typeof value.resolver.resolve !== "function") {
        throw new TypeError("Invalid recommendation account eligibility resolver.");
      }
      ...
      const normalized: RecommendationAccountCandidateEligibilityEvidence = {
        ...
        discoverable: booleanField(value.discoverable, "Invalid recommendation account discoverability evidence."),
        noindex: booleanField(value.noindex, "Invalid recommendation account noindex evidence."),
        optedOut: booleanField(value.optedOut, "Invalid recommendation account opt-out evidence.")
      };
...
function typeSpecificReasons(
  ...
  switch (evidence.kind) {
    case "account":
      ...
      if (!evidence.discoverable) reasons.push("account_not_discoverable");
      if (evidence.noindex) reasons.push("account_noindex");
      if (evidence.optedOut) reasons.push("account_opted_out");
      break;
    case "post":
...
export async function evaluateRecommendationCandidateEligibility(
  ...
  const providerPolicy = normalizeProviderPolicy(input.providerPolicy);
  const viewerSafety = normalizeViewerSafety(input.viewerSafety);
  ...
  if (candidate.availability === "unavailable") reasons.push("candidate_unavailable");
  if (!providerPolicy.evidenceComplete) reasons.push("provider_policy_incomplete");
  else if (!providerPolicy.allowed) reasons.push("provider_policy_denied");
  if (!viewerSafety.evidenceComplete) reasons.push("viewer_safety_incomplete");
  else if (!viewerSafety.eligible) reasons.push("viewer_safety_denied");
  reasons.push(...typeSpecificReasons(candidate, evidence));

  let resolvedAccount: RecommendationAccountProfile | undefined;
  let moveChain: readonly string[] | undefined;
  if (evidence.kind === "account" && !reasons.includes("identity_verification_required")) {
    ...
    if (!accountResult.eligible) reasons.push(accountReasonCode(accountResult.reason));
